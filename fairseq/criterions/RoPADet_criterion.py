...
@dataclass
class RoPADetCriterionConfig(FairseqDataclass):
    ...
    gamma: float = field(
        default= 0.0,
        metadata = {
            'help': 'gamma value for focal loss, 0.0 equals to Cross Entropy loss'
        }
    )
    loss_weights: List[float] = field(
        default_factory=lambda : [1],
        metadata={
            "help": "loss weights for each term in the loss function"
        },
    )

@register_criterion("RoPADet", dataclass=RoPADetCriterionConfig)
class RoPADetCriterion(FairseqCriterion):
    def __init__(self, task, sentence_avg, class_weights, ovr_onehot, gamma, loss_weights):
        super().__init__(task)
    ...
    def forward(self, model, sample, reduce=True):
        ...
        logging_output = {
            ...
            # "loss2": unsup_contrast_loss.item(),
