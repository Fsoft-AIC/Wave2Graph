...
class Wav2Vec2Model(BaseFairseqModel):
    ...
    def sample_negatives_with_additional_mask(self, y, num, batch_mask, padding_count=None):
        # batch_mask: sample with same id in the same batch should not be used as negative for each other
        # example: [0, 1, 2, 0, 3, 1, 4, 1]
        ...
        intervals = batch_mask[order]
        from itertools import groupby
        intervals = [sum(1 for _ in group) for _, group in groupby(intervals)]
        ...
        with torch.no_grad():
            if self.cross_sample_negatives > 0:
                ...
                for i in range(len(intervals)):
                    curr_high = intervals[i]*tsz
                    if i == 0:
                        # low = 0
                        if len(intervals) == 1:
                            curr_high = 0
                        curr_bsz = intervals[i]
                        curr_cross_neg_idxs = torch.randint(
                            low=curr_high,
                            high=cross_high-1,
                            size=(curr_bsz, self.cross_sample_negatives * num),
                        )
                    elif i == len(intervals) - 1:
                        curr_low = intervals[i-1]*tsz
                        curr_bsz = intervals[i] - intervals[i-1]
                        curr_cross_neg_idxs = torch.randint(
                            low=0,
                            high=curr_low - 1,
                            size=(curr_bsz, self.cross_sample_negatives * num),
                        ...
                        curr_low = intervals[i-1]*tsz
                        curr_bsz = intervals[i] - intervals[i-1]
                        lower_half_num_samples = int(self.cross_sample_negatives * num * curr_low/(curr_low+cross_high-curr_high))
                        higher_half_num_samples = self.cross_sample_negatives * num - lower_half_num_samples
                        curr_cross_neg_idxs_lower_half = torch.randint(
                            low=0,
                            high=curr_low - 1,
                            size=(curr_bsz, lower_half_num_samples),
                        )
                        curr_cross_neg_idxs_higher_half = torch.randint(
                            low=curr_high,
                            high=cross_high - 1,
                            size=(curr_bsz, higher_half_num_samples),
                        )
                        curr_cross_neg_idxs = torch.cat([curr_cross_neg_idxs_lower_half, curr_cross_neg_idxs_higher_half], dim=1)
                    curr_cross_neg_idxs[curr_cross_neg_idxs >= tszs] += 1
                    cross_neg_idxs.append(curr_cross_neg_idxs)
                cross_neg_idxs = torch.cat(cross_neg_idxs, axis=0)
        ...
        # print("CROSS NEG INDEX SHAPE: ", cross_neg_idxs.shape)
        # print("NEG INDEX SHAPE: ", neg_idxs.shape)
        # print("Y SHAPE: ", y.shape)
        negs = y[neg_idxs.view(-1)]
        negs = negs.view(
            bsz, num, self.n_negatives + self.cross_sample_negatives, fsz
        )
        reverse_order = order.argsort()
        # negs = negs.to('cpu')
        # print("NEGS SHAPE: ", negs.shape)
        # print("REVERSE ORDER: ", reverse_order)
        negs = negs[reverse_order,...]
        negs = negs.permute(
            2, 0, 1, 3
        )  # to NxBxTxC
        # negs = negs.to('cuda')
        neg_idxs = neg_idxs[reverse_order,...]
    ...
    def sample_negatives(self, y, num, padding_count=None):
        ...
        with torch.no_grad():
            ...
            if self.cross_sample_negatives > 0:
                ...
                # print("BUFFER: ", buffered_arange(num))
                # print("TSZS: ", tszs)
    ...
    def forward(
        ...
        **kwargs
        ...
        # print("SOURCE INPUT TYPE: ", type(source))
        ...
        if self.quantizer:
            ...
            if self.negatives_from_everywhere:
                # print("SAMPLE NEGATIVE FROM EVERYWHERE")
                # print("UNMASKED_FEATURES SHAPE: ", unmasked_features.shape)
                # print("Y SHAPE: ", y.shape)
                if 'batch_mask' in kwargs.keys():
                    negs, _ = self.sample_negatives_with_additional_mask(
                        unmasked_features,
                        y.size(1),
                        kwargs['batch_mask'],
                        padding_count=padding_count,
        ...
        if not is_xla_tensor(x):
            ...
            # print("X AFTER MASK INDICES SHAPE: ", x.shape)

        if self.target_glu:
            y = self.target_glu(y)
            negs = self.target_glu(negs)

        # print("OUTPUT SHAPE 1: ", x.shape)
        x = self.final_proj(x)
        # print("OUTPUT SHAPE 2: ", x.shape)
        x = self.compute_preds(x, y, negs)
        # print("OUTPUT SHAPE 3: ", x.shape)
    ...
    def remove_pretraining_modules(self, last_layer=None):
        self.quantizer = None
        # NOTE: temporary disable the below lines
        # self.project_q = None
        self.target_glu = None
