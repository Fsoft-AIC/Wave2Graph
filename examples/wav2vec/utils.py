...
def icbhi_evaluate(final_predicts, final_targets, args):
    ...
    print(class_counts)
    accuracy = accuracy_score(final_targets, final_predicts)
    ...
    print(f"ICBHI score: {icbhi_score:.4f}", file=f)

    run = args.run
    update_run(run, f'test_{args.input_file}', dict())
    run.summary[f'test_{args.input_file}']['accuracy'] = round(accuracy, 4)
    run.summary[f'test_{args.input_file}']['icbhi_score'] = round(icbhi_score, 4)
    run.summary.update()
            ...
            for line, label in tqdm(zip(f.read().split('\n')[1:-1], label.read().split('\n'))):
                if profiling:
                    try:
                        ...
                        if 'coswara' in file_path:
                            s = file_path
                            s = s[:s.rfind('/')]
                            profile_id = s[s.rfind('/')+1:]
                        else:
